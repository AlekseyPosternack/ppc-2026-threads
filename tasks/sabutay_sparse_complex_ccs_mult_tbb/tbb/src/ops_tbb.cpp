...
struct ThreadWorkspace {
  explicit ThreadWorkspace(int rows) : accumulator(static_cast<std::size_t>(rows), Complex{}),
                                       marker(static_cast<std::size_t>(rows), -1) {}
...
SparseMatrixCCS SabutayASparseComplexCcsMultTBB::MultiplyTbb(const SparseMatrixCCS& lhs, const SparseMatrixCCS& rhs) {
  ...
  oneapi::tbb::enumerable_thread_specific<ThreadWorkspace> workspaces(
      [&lhs]() { return ThreadWorkspace(lhs.rows); });

  oneapi::tbb::parallel_for(oneapi::tbb::blocked_range<int>(0, rhs.cols), [&](const oneapi::tbb::blocked_range<int>& range) {
